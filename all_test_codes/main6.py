...
class GoatHeuristicBot:
    ...
    def evaluate_placement(self, node, pieces, move_count):
        score = 0
        
        # Heuristic-based scoring
        if len(pieces['tigers']) < 3 and move_count < 10:
            potential_tiger_spots = set(range(len(self.nodes))) - set(pieces['goats']) - set(pieces['tigers'])
            if node in potential_tiger_spots:
                score += 500

        # Distribution score
        distribution_score = self.evaluate_distribution(node, pieces)
        score += distribution_score * 200

        # Avoid placing near tigers
        for tiger in pieces['tigers'].copy():
            if self.is_adjacent(tiger, node):
                score -= 1000

        # Opening table check
        key = (tuple(sorted(pieces['tigers'])), tuple(sorted(pieces['goats'])), node)
        if key in self.opening_table:
            score += self.opening_table[key]

        return score

    def evaluate_distribution(self, node, pieces):
        goat_positions = pieces['goats']
        avg_x = sum(self.nodes[pos][0] for pos in goat_positions) / len(goat_positions) if goat_positions else 0
        avg_y = sum(self.nodes[pos][1] for pos in goat_positions) / len(goat_positions) if goat_positions else 0
        
        node_x, node_y = self.nodes[node]
        distance_from_center = ((node_x - avg_x) ** 2 + (node_y - avg_y) ** 2) ** 0.5
        
        return distance_from_center

    def is_valid_position(self, node, pieces):
        return node not in pieces['goats'] and node not in pieces['tigers']

    def place_goat(self, pieces, move_count):
        available_nodes = [node for node in range(len(self.nodes)) 
                           if self.is_valid_position(node, pieces)]
        if not available_nodes:
            return None, None

        best_node = max(available_nodes, key=lambda node: self.evaluate_placement(node, pieces, move_count))
        self.goats_to_place -= 1
        return None, best_node

    def make_move(self, pieces, move_count):
        if self.goats_to_place > 0:
            return self.place_goat(pieces, move_count)
        else:
            return self.move_goat(pieces, move_count)

    def move_goat(self, pieces, move_count):
        available_moves = []
        for goat in pieces['goats']:
            for next_node in self.move_connections[goat]:
                if next_node not in pieces['tigers'] and next_node not in pieces['goats']:
                    available_moves.append((goat, next_node))

        if not available_moves:
            return None, None

        best_move = max(available_moves, key=lambda move: self.evaluate_move(move, pieces, move_count))
        return best_move

    def evaluate_move(self, move, pieces, move_count):
        goat, next_node = move
        score = 0

        # Prevent capture
        for tiger in pieces['tigers'].copy():
            if (tiger, next_node) in self.capture_connections[tiger]:
                score -= 1000

        # Create threats
        for opponent_piece in pieces['tigers']:
            if (opponent_piece, next_node) in self.capture_connections[opponent_piece]:
                score += 500

        # Opening table check
        key = (tuple(sorted(pieces['tigers'])), tuple(sorted(pieces['goats'])), (goat, next_node))
        if key in self.opening_table:
            score += self.opening_table[key]
...
def reset_game():
    global pieces, current_player, goats_to_place, selected_node, captured_goats, game_over, moves, move_count,tiger_agent
    pieces = {'tigers': [0], 'goats': []}
    current_player = 'goat'
    goats_to_place = 15
    selected_node = None
...
def main():
    global current_player, selected_node, goats_to_place, captured_goats, game_over, tiger_wins, goat_wins, move_count, tiger_agent,scroll_position,move_connections,node_positions
            ...
            if game_mode == 'Bot vs AI':
                if current_player == 'goat':
                    ...
                    if selected_node is None:  # Placement move
                        ...
                        goats_to_place -= 1
                        moves.append(f"Goat placed at node {node}")
                        print(f"Goat placed at node {node}")
                    else:  # Movement
                        pieces['goats'].remove(selected_node)
                        pieces['goats'].append(node)
                        moves.append(f"Goat moved from {selected_node} to {node}")
                        print(f"Goat moved from {selected_node} to {node}")
                        selected_node = None
                    move_count += 1
                    scroll_position = max(0, total_moves - max_visible_moves) + 1
                    current_player = 'tiger'
