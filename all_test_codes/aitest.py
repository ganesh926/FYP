...
class TigerHeuristicBot:
    ...
    def evaluate_move(self, move, pieces):
        start, end = move
        # Prioritize captures
        if any(self.capture_node.get((start, end_node)) in pieces['goats'] for end_node in self.capture_connections[start]):
            return 1000
        # Prioritize moves towards the center
        center_nodes = [0, 2, 3, 4, 5, 8, 9, 10, 11, 14, 15, 16, 17]
        if end in center_nodes:
            return 50
        # Prioritize moves that block goats
        blocked_goats = sum(1 for adj in self.capture_connections[end] if adj in pieces['goats'])
        return blocked_goats * 10
    ...
    def get_best_move(self, pieces):
        ...
        # Return the best move, or a random one from the top 3 if there are multiple good options
        best_score = evaluated_moves[0][1]
        best_moves = [move for move, score in evaluated_moves if score == best_score]
...
moves = []
options = ['Goat vs Tiger', 'Goat vs AI', 'Goat vs Bot']
selected_index=0
game_mode = options[0]
    ...
    if current_player == 'tiger':
        ...
        for i in range(len(capture_connections[start_node])):
            if end_node == capture_connections[start_node][i]:
                ...
                if intermediate_node in pieces['goats'] and end_node not in pieces['tigers']:
