...
class GoatHeuristicBot:
    def __init__(self, connections, capture_connections, capture_node, move_connections, node_positions):
        ...
        self.max_depth = 4
        self.central_nodes = [2, 3, 4, 5, 8, 9, 10, 11, 14, 15, 16, 17]
        self.corner_nodes = [0,1, 6, 13, 18, 19, 22]
        self.move_history = []  # New: Store recent moves
        self.max_history = 20  # New: Maximum number of moves to remember
    ...
    def place_goat(self, pieces, goats_to_place):
        ...
        # Prioritize corner nodes for early placements
        if goats_to_place >= 10:
            corners_available = [node for node in self.corner_nodes if node in available_nodes]
            if corners_available:
                placement = random.choice(corners_available)
                self.update_move_history(None, placement)
                return None, placement
        
        # Prioritize central nodes for later placements
        if goats_to_place < 10:
            central_available = [node for node in self.central_nodes if node in available_nodes]
            if central_available:
                placement = random.choice(central_available)
                self.update_move_history(None, placement)
                return None, placement
        
        # If no prioritized nodes are available, use minimax
        ...
        for node in available_nodes:
            if self.is_move_in_history(None, node):
                continue  # Skip this move if it's in recent history
            new_pieces = deepcopy(pieces)
            new_pieces['goats'].append(node)
            score = self.minimax(new_pieces, goats_to_place - 1, self.max_depth, alpha, beta, False)
            if score > best_score:
                best_score = score
                best_move = node
        ...
        self.update_move_history(None, best_move)
        ...
        for goat in pieces['goats']:
            for next_node in self.move_connections[goat]:
                if next_node not in pieces['tigers'] and next_node not in pieces['goats']:
                    if self.is_move_in_history(goat, next_node):
                        continue  # Skip this move if it's in recent history
                    new_pieces = deepcopy(pieces)
                    new_pieces['goats'].remove(goat)
                    new_pieces['goats'].append(next_node)
                    score = self.minimax(new_pieces, 0, self.max_depth, alpha, beta, False)
                    if score > best_score:
                        best_score = score
    ...
    def evaluate_board(self, pieces, goats_to_place):
        score = 0
        ...
        captured_goats = 15 - len(pieces['goats']) - goats_to_place
        ...
        goats_in_center = sum(1 for node in self.central_nodes if node in pieces['goats'])
        score += goats_in_center * 5
        
        # Avoid corner nodes
        goats_in_corner = sum(1 for node in self.corner_nodes if node in pieces['goats'])
        score += goats_in_corner * 10
        
        # Trapping tigers evaluation
        score += self.evaluate_tiger_trapping(pieces) * 30
        
        # Defensive formation around tigers
        score += self.evaluate_defensive_formation(pieces) * 25
    ...
    def evaluate_goat_formation(self, pieces):
        ...
        for goat in pieces['goats']:
            ...
            formation_score += adjacent_goats * 5  # Encourage clustering
    ...
    def evaluate_defensive_formation(self, pieces):
        defensive_score = 0
        for tiger in pieces['tigers']:
            surrounding_goats = sum(1 for adj in self.move_connections[tiger] if adj in pieces['goats'])
            if surrounding_goats >= 2:
                defensive_score += 20 * surrounding_goats
...
def draw_game_over_popup():
    ...
    back_text = info_font.render("Press 'B' to go Back to Menu", True, BLACK)
    ...
    screen.blit(winner_text, winner_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 40)))
    screen.blit(reset_text, reset_text.get_rect(center=(WIDTH // 2, HEIGHT // 2)))
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if not game_over and current_screen == GAME_SCREEN and event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
        ...
        if current_screen == GAME_SCREEN and not game_over:
